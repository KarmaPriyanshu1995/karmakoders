...
/** Collect every indexable public URL from the database for a tenant. */
export async function collectSitemapEntries(tenantId: string): Promise<SitemapUrlEntry[]> {
  const [pages, posts, projects, jobs, toolEntries] = await Promise.all([
    prisma.page.findMany({ where: { tenantId, isPublished: true }, select: { slug: true } }),
    prisma.post.findMany({ where: { tenantId, published: true }, select: { slug: true, createdAt: true } }),
    prisma.project.findMany({ where: { tenantId }, select: { slug: true, createdAt: true } }),
    prisma.jobOpening.findMany({ where: { tenantId, isActive: true }, select: { slug: true, createdAt: true } }),
  ...
  for (const page of SITE_PAGES) {
    if (SITEMAP_EXCLUDED_PAGE_SLUGS.has(page.slug)) continue;
  ...
  for (const post of posts) {
    ...
    if (!post.slug || post.slug.startsWith("-")) continue;
    entries.push(
  ...
  for (const tool of toolEntries) {
    const path = tool.url.startsWith(SITE_URL) ? tool.url.slice(SITE_URL.length) || "/" : tool.url;
    entries.push(
      entry(path, "free-tools", {
        changeFrequency: tool.changeFrequency,
        lastModified: tool.lastModified instanceof Date ? tool.lastModified : undefined,
        priority: 0.85,
      })
    );
  }
