  ...
  return (
    ...
    <section id="case-studies" aria-label="Case studies" className={`${(isFirstSection || !isCentered) && "pt-28 sm:pt-32"} pb-20 sm:pb-32 px-4 sm:px-6 md:px-12 bg-slate-950 relative overflow-hidden`}>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className={`flex flex-col md:flex-row md:items-end justify-between items-center mb-12 gap-8`}>
          <div className={`text-center max-w-2xl`}>
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 text-indigo-500 text-sm font-bold tracking-widest uppercase"
            >
              {tagline}
            </motion.div>
            {isFirstSection ? (
            ...
            >
              <Link href={`/blog/${item.slug || "#"}`}>
                <div className="relative rounded-[2rem] overflow-hidden aspect-[16/10] mb-8">
                  ...
                  />
                  {/* Consistent brand overlay instead of a random per-card color gradient */}
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent" />
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ring-1 ring-inset ring-indigo-500/40 rounded-[2rem]" />

                  {isFallback && (
                    <span className="absolute top-6 left-6 px-2.5 py-1 bg-slate-950/70 backdrop-blur-sm border border-white/20 text-white/70 text-[10px] font-bold uppercase tracking-wider rounded-md">
                      Concept
                    </span>
                  )}
