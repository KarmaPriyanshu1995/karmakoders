...
interface AddMemberFormProps {
  createAction: (input: { name: string; email: string; role: MembershipRole }) => Promise<{ tempPassword: string | null }>;
}
...
export function AddMemberForm({ createAction }: AddMemberFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { tempPassword } = await createAction({ name, email, role });
      if (tempPassword) {
        setResult(tempPassword);
      } else {
        toast.success(`${email} added to this tenant`);
        setOpen(false);
        reset();
      }
    } catch (error) {
